...
function getInitialUrlState() {
  ...
  return {
    mode: ["simple", "detail", "senior"].includes(nextMode) ? nextMode : defaults.mode,
    language: ["ko", "en"].includes(nextLang) ? nextLang : defaults.language,
    region: nextRegion || defaults.region,
  };
}
...
function App() {
  ...
  function changeMapZoom(delta) {
    setMapZoom((zoom) => Math.max(1, Math.min(2.2, Number((zoom + delta).toFixed(2)))));
  }
      ...
      <section className="dashboard-layout">
        ...
        <aside className="right-panel">
          <div className="results-head">
            ...
            <p>{isEnglish ? "Calculated locally from mock data and the five category weights." : "mock 데이터를 기반으로 5개 카테고리 가중치를 적용해 계산되었습니다."}</p>
          </div>
